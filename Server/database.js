...
export async function getTodoByID (id) {
    const [row] = await pool.query(
        ...
        WHERE todos.user_id = ? OR shared_todos.shared_with_id = ?`, [id, id]);
    return row;
    /* console.log(row[0]) */
};
...
export async function deleteTodo(todo) {
    const [result] = await pool.query(`DELETE FROM todos WHERE id = ?`, [todo]);
    return result;
}
...
export async function shareTodo(todo_id, user_id, shared_with_id) {
    const [result] = await pool.query(`INSERT INTO shared_todos (todo_id, user_id, shared_with_id) VALUES (?, ?, ?)`, 
